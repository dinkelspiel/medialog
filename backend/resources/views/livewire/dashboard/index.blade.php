        ...
        <div>
            @foreach ($userEntries as $browserEntry)
                @if (!is_null($browserEntry->entry->franchise))
                    <x-entry :entry="$browserEntry->entry" :rating="$browserEntry->getLatestCompleted() ? $browserEntry->getLatestCompleted()->rating : null" wire:click="showUserEntry({{ $browserEntry->id }})" />
                @endif
            @endforeach
        </div>
    </div>
